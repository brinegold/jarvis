  ...
  try {
    ...
    if (transferType === 'main-to-fund') {
      ...
      try {
        ...
        // Get receiver email
        const { data: receiverAuth } = await supabase.auth.admin.getUserById(receiver.id)
    ...
    try {
      ...
      if (user) {
        ...
        console.log("Transfer failure email sent")
      }
    } catch (emailError) {
      console.error("Failed to send transfer failure email:", emailError)
    }
